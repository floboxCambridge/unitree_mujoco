...
class Go2Controller:
    ...
    def distribute_3d_forces(self, Fx, Fy, Fz, Tx, Ty, Tz, com, foot_positions):
        """
        Full 3D force distribution.

        Inputs:
            Fx, Fy, Fz : desired body force
            Tx, Ty, Tz : desired body torque
            com        : COM position, shape (3,)
            foot_positions : list of 4 foot positions, each shape (3,)

        Output:
            foot_forces : list of 4 forces [Fx_i, Fy_i, Fz_i]
        """

        W_des = np.array([Fx, Fy, Fz, Tx, Ty, Tz])
        ...
        # Minimum-norm damped solution:
        # f = A.T (A A.T + lambda I)^-1 W
        f = A_w.T @ np.linalg.inv(
            A_w @ A_w.T + damping * np.eye(6)
        ) @ W_w
        ...
        for i in range(4):
            ...
            # Contact cannot pull ground
            Fi[2] = np.clip(Fi[2], 0.0, fz_max)

            # Friction cone approximation
            horizontal_norm = np.linalg.norm(Fi[0:2])
            max_horizontal = mu * Fi[2]

            if horizontal_norm > max_horizontal and horizontal_norm > 1e-6:
                Fi[0:2] *= max_horizontal / horizontal_norm

            foot_forces.append(Fi)
    ...
    def run(self):
        ...
        while True:
            ...
            if t < self.time_up:
                ...
                foot_forces = self.distribute_3d_forces(
                    Fx, Fy, Fz, Tx, Ty, Tz,
                    com_pos,
                    foot_pos
                )
            ...
            com_x,com_y, com_z = self.estimate_com_position()
            print(f"com position: ({com_x:.2f}, {com_y:.2f}, {com_z:.2f})m")
