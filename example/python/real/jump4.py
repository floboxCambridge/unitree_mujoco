...
class Go2Jump4Controller:
    def __init__(self, interface=None, sim=True):
        ...
        self.jump_legs = ["FR", "FL", "RR", "RL"]
        self.fr_bend_home = {leg: None for leg in self.jump_legs}

        # Virtual spring parameters for bending the front legs
        ...
        self.fr_compression_distance = 0.25 # 15 cm
        self.fr_extension_distance = 0.35   # 35 cm
        self.fr_done_distance = 0.3

        # Reference while compressing the front legs.
        ...
        # Reference while jumping/extending the front legs.
    ...
    def add_fr_bend_spring_torque(self, tau_all, phase):
        for leg in self.jump_legs:
            ...
            # Save each front foot position at the moment bending starts
            if self.fr_bend_home[leg] is None:
                self.fr_bend_home[leg] = p_leg.copy()

            # Desired front foot position.
            # Positive x  -> forward
            # Right/left y stays at each leg home position from fk_leg()
            ...
            target_pos = self.fr_bend_home[leg] + np.array([0., 0., fr_bend_offset])

            # Virtual spring-damper force
            if phase == "compress":
                self.fr_bend_kp = np.diag([-100.0, -300.0, 300.0])
                self.fr_bend_kd = np.diag([6.0, 6.0, 30.0])
            elif phase == "jump":
                self.fr_bend_kp = np.diag([-80.0, -200.0, 2000.0])
            elif phase == "done":
                self.fr_bend_kp = np.diag([-100.0, -300.0, 300.0])
            ...
            if leg in ["RR", "RL"]:
                spring_force[2] *= self.springs_g_rear_scale.get(phase, 1.0)
    ...
    def run(self):
        ...
        while True:
            ...
            if elapsed < self.time_up: #first phase:ruse if the robot
                ...
                front_distances = {}
                for leg in self.jump_legs:
                    leg_idx = 3 * self.leg_index[leg]
                    leg_pose = self.fk_leg(self.joint_q[leg_idx:leg_idx + 3], leg)
                    front_distances[leg] = leg_pose[2] #z position of the foot
                ...
                if seq_t < self.initial_stabilize_duration: #four legs just switch to vmc controller (position control to torque control)
                    ...
                    for leg in self.jump_legs:
                        leg_idx = 3 * self.leg_index[leg]
                        leg_pose = self.fk_leg(self.joint_q[leg_idx:leg_idx + 3], leg)
                        front_distances[leg] = abs(leg_pose[2]) #z position of the foot
                    ...
                    if self.fr_phase == "compress":
                        self.announce_phase("four_leg_compress_spring")
                        ...
                        # Add compression spring on all four legs
                        tau_all = self.add_fr_bend_spring_torque(tau_all, "compress")

                        # Switch to jump once all four legs are compressed enough.
                        if all(distance <= self.fr_compression_distance for distance in front_distances.values()):
                            print(
                                "Four-leg compression reached: "
                                + ", ".join(f"{leg}={distance:.3f} m" for leg, distance in front_distances.items())
                            )
                            self.fr_phase = "jump"

                    elif self.fr_phase == "jump":
                        self.announce_phase("four_leg_jump_spring")
                        ...
                        # Set reference much farther so all four legs extend hard
                        self.fr_bend_offset = self.fr_jump_offset.copy()

                        # Add jump spring on all four legs
                        tau_all = self.add_fr_bend_spring_torque(tau_all, "jump")

                        # Remove contractile element once all four legs reach 35 cm.
                        if all(distance >= self.fr_extension_distance for distance in front_distances.values()):
                            print(
                                "Four-leg extension reached: "
                                + ", ".join(f"{leg}={distance:.3f} m" for leg, distance in front_distances.items())
                            )
                            self.fr_phase = "done"
                        ...
                        if all(distance <= self.fr_done_distance +0.01 for distance in front_distances.values()):
                            print(
                                "Four-leg settling reached: "
                                + ", ".join(f"{leg}={distance:.3f} m" for leg, distance in front_distances.items())
                            )
                            self.fr_phase = "over"
                        ...
                        # Only normal 4-leg stabilization, no front spring
                        tau_all = self.compute_stance_torques(self.leg_names)
...
if __name__ == "__main__":
    ...
    controller = Go2Jump4Controller(interface=interface, sim=sim)
    try:
        controller.run()
