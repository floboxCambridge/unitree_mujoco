...
from runtime_bootstrap import confirm_real_robot_start, ensure_project_python
...
class Go2FourLegSequenceController(Go2FRForwardStepController):
    ...
    def run(self):
        ...
        # Duration of the repeated walking part only.
        # The initial stabilization happens once, then the leg sequence repeats forever.
        repeated_sequence_duration = total_sequence_duration - self.initial_stabilize_duration
        ...
        while True:
            ...
            if elapsed < self.time_up:
                ...
                raw_seq_t = t - sequence_start
                tau_all = np.zeros(12)

                # Stabilize once at the beginning, then repeat the walking sequence forever.
                if raw_seq_t < self.initial_stabilize_duration:
                    seq_t = raw_seq_t
                else:
                    seq_t = (
                        self.initial_stabilize_duration
                        + ((raw_seq_t - self.initial_stabilize_duration) % repeated_sequence_duration)
                    )
...
if __name__ == "__main__":
    sim = "--real" not in sys.argv[1:]
    args = [arg for arg in sys.argv[1:] if arg != "--sim" and arg != "--real"]
    interface = args[0] if args else None

    if not sim:
        confirm_real_robot_start("REAL ROBOT LOW-LEVEL CONTROL. Press Enter to continue.")
