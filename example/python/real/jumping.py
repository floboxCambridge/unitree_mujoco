...
class Go2FRForwardStepController:
    def __init__(self, interface=None, sim=True):
        ...
        self.fr_bend_offset = np.array([0.0, -0.0, -0.2])
        self.fr_bend_force_limit = 50.0
        self.fr_bend_tau_limit = 30.0
        self.fr_bend_force_limit = 35.0
    ...
    def add_fr_bend_spring_torque(self, tau_all):
        leg = "FR"
        idx = 3 * self.leg_index[leg]
        ...
        target_pos = self.fr_bend_home + self.fr_bend_offset
        ...
        while True:
            ...
            if elapsed < self.time_up: #first phase:ruse if the robot
                ...
                if seq_t < self.initial_stabilize_duration: #four legs just switch to vmc controller (position control to torque control)
                    ...
                    self.announce_phase("fr_bend_spring")
                    self.K_pitch = 0.
                    self.D_pitch = 0.
                    self.K_roll = 0.
                    self.D_roll = 0.
                    ...
                    # Add extra virtual spring force on FR
                    tau_all = self.add_fr_bend_spring_torque(tau_all)
